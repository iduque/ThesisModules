...
class ZigBeeThread(Thread):
  ...
  def handler_temperature_mcp9700_hot_status(self, channel_uuid, value):
    ...
    filter_length = 10 # Moving Average Filter of length 20
    try:
    ...
    if (status == False and temp > (1.01 * avg)):
      status = True
    elif (status == True and temp < avg):
      status = False
    if status == True:
      return 'On'
    else:
  ...
  def handler_temperature_mcp9700_cold_status(self, channel_uuid, value):
    ...
    if len(valmem) > filter_length:
      valmem.pop(0)
    avg = sum(valmem) / len(valmem)
    if (status == False and temp < (0.99 * avg)):
      status = True
    elif (status == True and temp > avg):
      status = False
    if status == True:
...
class LoggerThread(Thread):
  ...
  def run(self):
    ...
    while self.status:
      ...
      for k in key:
        try:
          if logged_channels[k]['value'] != channels[k]['value'] or logged_channels[k]['status'] != channels[k]['status']:
