...
class UserInterface(object):
    ...
    def setResponse(self, sequenceName, responseId, repeated = 0):
        sql = 'UPDATE `%s` ' % (self._guiQuestionTable)
        sql += 'SET `guiMsgResult` = %(resp)s, `repeated` = %(repeated)s WHERE `name` = %(seqName)s'
        args = { 
                'resp': responseId,
...
class DataAccess(object):
    ...
    def setResponse(self, sequenceName, responseId, repeated = 0):
        return self.userInterface.setResponse(sequenceName, responseId, repeated)
