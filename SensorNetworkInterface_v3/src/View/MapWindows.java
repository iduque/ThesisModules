...
public class MapWindows extends javax.swing.JFrame {
    ...
    public MapWindows(Controller c) {
        ...
        this.Office.addMouseListener(new MyMouseListener("Office"));
        this.Bedroom.addMouseListener(new MyMouseListener("Bedroom"));
        this.Kitchen.addMouseListener(new MyMouseListener("Kitchen"));
        this.Bathroom.addMouseListener(new MyMouseListener("Bathroom"));
        this.LivingRoomTable.addMouseListener(new MyMouseListener("Dining_Area"));
        this.LivingRoomSofa.addMouseListener(new MyMouseListener("Living_Room"));
        
        //Set cursor when the mouse is over the component
        this.Hall.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        this.Office.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
        ...
        //Deactivate the button until some user name is introduced
        this.jbStartStopRecording.setEnabled(false);
    ...
    private void initComponents() {
        ...
        jl_K19.setFont(new java.awt.Font("Tahoma", 0, 8)); // NOI18N
        ...
        Office.setBounds(270, 400, 190, 190);
        JLayerMap.add(Office, javax.swing.JLayeredPane.DEFAULT_LAYER);
        ...
        jlPreviousActivities.setText("Previous Activities");
    ...
    private void jbStartStopRecordingActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbStartStopRecordingActionPerformed
        if(controller.getStatusRecording()){
            controller.stopRecording(); //Set recording status to false
            this.jbStartStopRecording.setText("Start Recording");
        }else{
            controller.startRecording(); //Set recording status to true
    ...
    private void jtfUsernameKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_jtfUsernameKeyReleased
        ...
        if(txt.getText().equals("")) this.jbStartStopRecording.setEnabled(false);
        else{//Enable button and set user in the controller class
           controller.setUserName(txt.getText());
           this.jbStartStopRecording.setEnabled(true);
        }
    }//GEN-LAST:event_jtfUsernameKeyReleased
    
    /**
     * Set up the table columns which will appear on the interface
     */
    public void setData (String module, String channel, String id, String name, String type,
   ...
   /**
     ...
     */ 
   public void initializeTable(SensorsXMLModel data){
       
       Iterator itData = data.getData().keySet().iterator();               
       String sensorsInfo = "";
               
       while(itData.hasNext()){

            String tab = (String) itData.next();
            //Add the sensor location information to the text area
            sensorsInfo += tab + "\n";
            
            LinkedList adaptedData = new LinkedList();

            Iterator itModules = data.getData().get(tab).keySet().iterator();
            while(itModules.hasNext()){

                String mac = (String) itModules.next();
                Module m =  data.getData().get(tab).get(mac);
                String moduleName = m.getModuleName();
                
                Iterator itModule = m.getSensors().keySet().iterator();
                while(itModule.hasNext()){
                    
                    String channel = (String) itModule.next();
                    adaptedData.add(m.getSensors().get(channel).getId()); //ID
                    
                    //Add the sensor id information to the text area
                    sensorsInfo += "--> " + m.getSensors().get(channel).getId() + " - ";
                    
                    adaptedData.add(moduleName); //Module
                    adaptedData.add(channel); //Channel
                    adaptedData.add(m.getSensors().get(channel).getName()); //Name
                    
                    //Add the sensor name information to the text area
                    sensorsInfo +=  m.getSensors().get(channel).getName() + "\n";
                    
                    adaptedData.add(m.getSensors().get(channel).getType()); //Type
                    adaptedData.add("-"); //Value
                    adaptedData.add("-"); //Status
            ...
            this.JTabbedData.add(tab, scrollpane);
    ...
    * @param tab Room in which the sensor can be found
   ...
   public Boolean setValueCell(String tab, String idSensor, String value, String status){
           ...
           JTableModel auxTable = ((JTableModel)
                   ((JTable)
                   ((JScrollPane) this.JTabbedData.getComponentAt(_labels.indexOf(tab))).getViewport().getView())
                   .getModel());

           int pos = auxTable.getPositionSensor(idSensor);
           ...
           if (pos != -1 ){
               ...
               //If the value is the same that before, the system doesn´t make any change
               if(!status.equals(auxTable.getValueAt((pos/auxTable.getColumnCount()), _columns.indexOf(STATUS_COLUMN)))){

                   auxTable.setValueAt(status, pos/auxTable.getColumnCount(), _columns.indexOf(STATUS_COLUMN));
                   refreshSensorMap(status, Integer.parseInt(idSensor));
  ...
  private void refreshSensorMap(String status, int id) {
        ...
        switch (id){
            ...
            //Kitchen
            case 1: if(open?changeLabelColor(Color.GREEN,jl_K1):changeLabelColor(Color.RED,jl_K1));
                    break;
            case 2: if(open?changeLabelColor(Color.GREEN,jl_K2):changeLabelColor(Color.RED,jl_K2));
                    break;
            case 3: if(open?changeLabelColor(Color.GREEN,jl_K3):changeLabelColor(Color.RED,jl_K3));
                    break;
            case 4: if(open?changeLabelColor(Color.GREEN,jl_K4):changeLabelColor(Color.RED,jl_K4));
                    break;
            case 5: if(open?changeLabelColor(Color.GREEN,jl_K5):changeLabelColor(Color.RED,jl_K5));
                    break;
            case 6: if(open?changeLabelColor(Color.GREEN,jl_K6):changeLabelColor(Color.RED,jl_K6));
                    break;
            case 7: if(open?changeLabelColor(Color.GREEN,jl_K7):changeLabelColor(Color.RED,jl_K7));
                    break;
            case 8: if(open?changeLabelColor(Color.GREEN,jl_K8):changeLabelColor(Color.RED,jl_K8));
                    break;
            case 9: if(open?changeLabelColor(Color.GREEN,jl_K9):changeLabelColor(Color.RED,jl_K9));
                    break;
            case 10: if(open?changeLabelColor(Color.GREEN,jl_K10):changeLabelColor(Color.RED,jl_K10));
                    break;
            case 58: if(open?changeLabelColor(Color.GREEN,jl_K44):changeLabelColor(Color.RED,jl_K44));
                    break;
            case 50: if(open?changeLabelColor(Color.GREEN,jl_K50):changeLabelColor(Color.RED,jl_K50));
                    break;
            case 51: if(open?changeLabelColor(Color.GREEN,jl_K51):changeLabelColor(Color.RED,jl_K51));
                    break;
            case 54: if(open?changeLabelColor(Color.GREEN,jl_K54):changeLabelColor(Color.RED,jl_K54));
                    break;
            case 55: if(open?changeLabelColor(Color.GREEN,jl_K55):changeLabelColor(Color.RED,jl_K55));
                    break;
            case 56: if(open?changeLabelColor(Color.GREEN,jl_K56):changeLabelColor(Color.RED,jl_K56));
                    break;

            
            //Bathroom
            case 11: if(open?changeLabelColor(Color.GREEN,jl_K11):changeLabelColor(Color.RED,jl_K11));
                    break;    
            case 12: if(open?changeLabelColor(Color.GREEN,jl_K12):changeLabelColor(Color.RED,jl_K12));
                    break;
            case 13: if(open?changeLabelColor(Color.GREEN,jl_K13):changeLabelColor(Color.RED,jl_K13));
                    break;
            case 14: if(open?changeLabelColor(Color.GREEN,jl_K14):changeLabelColor(Color.RED,jl_K14));
                    break;
                
            //Living Room
            case 15: if(open?changeLabelColor(Color.GREEN,jl_K15):changeLabelColor(Color.RED,jl_K15));
                    break;    
            case 16: if(open?changeLabelColor(Color.GREEN,jl_K16):changeLabelColor(Color.RED,jl_K16));
                    break;
            case 17: if(open?changeLabelColor(Color.GREEN,jl_K17):changeLabelColor(Color.RED,jl_K17));
                    break;
            case 18: if(open?changeLabelColor(Color.GREEN,jl_K18):changeLabelColor(Color.RED,jl_K18));
                    break;
            case 19: if(open?changeLabelColor(Color.GREEN,jl_K19):changeLabelColor(Color.RED,jl_K19));
                    break;
            case 49: if(open?changeLabelColor(Color.GREEN,jl_K49):changeLabelColor(Color.RED,jl_K49));
                    break;

                
            //Dining Area
            case 20: if(open?changeLabelColor(Color.GREEN,jl_K20):changeLabelColor(Color.RED,jl_K20));
                    break;
            case 21: if(open?changeLabelColor(Color.GREEN,jl_K21):changeLabelColor(Color.RED,jl_K21));
                    break;
            case 22: if(open?changeLabelColor(Color.GREEN,jl_K22):changeLabelColor(Color.RED,jl_K22));
                    break;
            case 23: if(open?changeLabelColor(Color.GREEN,jl_K23):changeLabelColor(Color.RED,jl_K23));
                    break;
            case 24: if(open?changeLabelColor(Color.GREEN,jl_K24):changeLabelColor(Color.RED,jl_K24));
                    break;
            case 25: if(open?changeLabelColor(Color.GREEN,jl_K25):changeLabelColor(Color.RED,jl_K25));
                    break;
            case 26: if(open?changeLabelColor(Color.GREEN,jl_K26):changeLabelColor(Color.RED,jl_K26));
                    break;
            case 27: if(open?changeLabelColor(Color.GREEN,jl_K27):changeLabelColor(Color.RED,jl_K27));
                    break;
            case 28: if(open?changeLabelColor(Color.GREEN,jl_K28):changeLabelColor(Color.RED,jl_K28));
                    break;
            case 29: if(open?changeLabelColor(Color.GREEN,jl_K29):changeLabelColor(Color.RED,jl_K29));
                    break;
            case 30: if(open?changeLabelColor(Color.GREEN,jl_K30):changeLabelColor(Color.RED,jl_K30));
                    break;
            case 57: if(open?changeLabelColor(Color.GREEN,jl_K57) : changeLabelColor(Color.RED,jl_K57));
                    break;
                
            //Bedroom
            case 31: if(open?changeLabelColor(Color.GREEN,jl_K31):changeLabelColor(Color.RED,jl_K31));
                    break;
            case 32: if(open?changeLabelColor(Color.GREEN,jl_K32):changeLabelColor(Color.RED,jl_K32));
                    break;
            case 33: if(open?changeLabelColor(Color.GREEN,jl_K33):changeLabelColor(Color.RED,jl_K33));
                    break;
            case 34: if(open?changeLabelColor(Color.GREEN,jl_K34):changeLabelColor(Color.RED,jl_K34));
                    break;
            case 35: if(open?changeLabelColor(Color.GREEN,jl_K35):changeLabelColor(Color.RED,jl_K35));
                    break;
            case 36: if(open?changeLabelColor(Color.GREEN,jl_K36):changeLabelColor(Color.RED,jl_K36));
                    break;
            case 37: if(open?changeLabelColor(Color.GREEN,jl_K37):changeLabelColor(Color.RED,jl_K37));
                    break;
            case 38: if(open?changeLabelColor(Color.GREEN,jl_K38):changeLabelColor(Color.RED,jl_K38));
                    break;
            case 39: if(open?changeLabelColor(Color.GREEN,jl_K39):changeLabelColor(Color.RED,jl_K39));
                    break;
            case 40: if(open?changeLabelColor(Color.GREEN,jl_K40):changeLabelColor(Color.RED,jl_K40));
                    break;
            case 52: if(open?changeLabelColor(Color.GREEN,jl_K52):changeLabelColor(Color.RED,jl_K52));
                    break;
            case 53: if(open?changeLabelColor(Color.GREEN,jl_K53):changeLabelColor(Color.RED,jl_K53));
                    break;

             //Other
             case 42: if(open?changeLabelColor(Color.GREEN,jl_K42):changeLabelColor(Color.RED,jl_K42));
                    break;
             case 43: if(open?changeLabelColor(Color.GREEN,jl_K43):changeLabelColor(Color.RED,jl_K43));
                    break;
             case 59: if(open?changeLabelColor(Color.GREEN,jl_K59):changeLabelColor(Color.RED,jl_K59));
                    break;
  ...
  static class MyTableCellRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
        ...
        if(s.equals(CommonVar.ON)) foundvalue = true;
        
        if(foundvalue) c.setBackground(new Color(100, 230, 100));
